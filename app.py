...
def can_show_riddle():
    """
    Returns True if the current local time in Asia/Riyadh is between 9:00 PM and 9:05 PM.
    ...
    start_time = now.replace(hour=18, minute=0, second=24, microsecond=0)
    end_time   = now.replace(hour=18, minute=5, second=28, microsecond=0)
            ...
            if st.button("تحقق من الإجابة"):
                ...
                if is_correct:
                    ...
                    if correct_count == 0:
                        ...
                        add_points = 0  # All others get 3 points
